...
fn test_binary_op_in_block() {
    ...
    assert!(block.stmts.len() >= 3, "expected at least 3 statements (2 LocalDecls + 2 Assigns), got {}", block.stmts.len());
...
fn test_printf_call_as_terminator() {
    ...
    match &entry.terminator {
        MirTerminator::Call { func: _, args, destination: _, next } => {
...
fn test_if_else_cfg() {
    ...
    assert!(func.blocks.len() >= 4, "expected >= 4 blocks, got {}", func.blocks.len());
...
fn test_elif_chain_cfg() {
    let mir = to_mir("fn main() -> i64 { let x = 1 if x == 1 { 1 } elif x == 2 { 2 } else { 3 } return 0 }");
